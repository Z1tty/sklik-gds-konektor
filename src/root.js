  ...
  */
  ...
  this.logIn = function () {
    ...
    if (response && response.session) {
      ...
      this.Log.addRecord('Důvod chyby u přihlašování: ' + JSON.stringify(response));
      DataStudioApp.createCommunityConnector()
        .newUserError()
        .setText('Nepodařilo se přihlásit. Zkontrolujte prosím token a userId.')
        .setDebugText('Login failed. API response: ' + JSON.stringify(response))
        .throwException();
    }
  }
    ...
    try {
      ...
      if (stat.getResponseCode() == 200) {
        if (response.session) {
          ...
          return response;
          //Logout and login do not necessarily return session (just status and statusMessage)
        } else if (method == 'client.logout' || method == 'client.loginByToken') {
          return response;
        } else {
      ...
      if (retry <= 3) {
        ...
        DataStudioApp.createCommunityConnector()
          .newUserError()
          .setText('Nepodařilo se spojit s API Skliku. Zkuste to prosím později.')
          .setDebugText('sklikApiCall failed after 3 retries. Method: ' + method + ', Error: ' + JSON.stringify(exp))
          .throwException();
      }
    }
  }
